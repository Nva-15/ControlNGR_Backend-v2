...
Requisitos: Docker Desktop (o Docker Engine con el plugin `compose`).

```bash
cp .env.example .env        # completar DB_PASSWORD, DB_ROOT_PASSWORD y JWT_SECRET
...
- Los datos se guardan en volúmenes de Docker (`db_data`, `img_data` y `evidencias_data`), así que no se pierden al reiniciar.
- Para llevar el sistema a otro equipo basta con clonar el repositorio, crear el `.env` y ejecutar el mismo comando.
...
2. Crear el `.env` en la raíz del proyecto (Spring lo lee automáticamente) con `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD` y `JWT_SECRET`.
...
`parametros` · `segmentos-red` · `feriados` · `departamentos` · `tipos-usuario` · `reglas-aprobacion` · `tipos-solicitud` · `motivos-licencia` · `usuarios` (rol, estado, restablecer contraseña) · `saldos` (listado, `carga-inicial`, `ajuste`, movimientos) · `feriados-laborados` (listado y revertir) · `vacaciones/procesar`.
...
## 6. Si hay un proxy (nginx) delante del backend

Para que la validación de IP use la IP real del cliente, configure en el `.env`:

```
FORWARD_HEADERS_STRATEGY=native
TRUSTED_PROXIES=172\\.\\d+\\.\\d+\\.\\d+
```

Solo deben confiarse las IPs del proxy. Así ningún cliente puede falsificar su IP con el encabezado `X-Forwarded-For`.
